...
def calculate_correlation(*args):
    ...
    # Calculate rolling correlation for MoM (Month-over-Month) data
    for column in merged_data_mom.columns:
        if column != 'Inflation_Rate_MoM':  # Skip the CPI column itself
            # Calculate rolling correlation with CPI data
            merged_data_mom[f'Rolling_Correlation_{column}'] = merged_data_mom['Inflation_Rate_MoM'].rolling(window=window_size).corr(merged_data_mom[column])
            
            # Calculate smoothed rolling correlation by averaging over a 6-month window
            #merged_data_mom[f'Smoothed_Rolling_Correlation_{column}'] = merged_data_mom[f'Rolling_Correlation_{column}'].rolling(window=6).mean()

    # Calculate rolling correlation for YoY (Year-over-Year) data
    for column in merged_data_yoy.columns:
        if column != 'Inflation_Rate_YoY':  # Skip the CPI column itself
            # Calculate rolling correlation with CPI data
            merged_data_yoy[f'Rolling_Correlation_{column}'] = merged_data_yoy['Inflation_Rate_YoY'].rolling(window=window_size).corr(merged_data_yoy[column])
            
            # Calculate smoothed rolling correlation by averaging over a 6-month window
            #merged_data_yoy[f'Smoothed_Rolling_Correlation_{column}'] = merged_data_yoy[f'Rolling_Correlation_{column}'].rolling(window=6).mean()
    #merged_data.dropna(inplace=True)
    # Display the resulting DataFrame with rolling correlation values
 
    # Smoothing with a 6-month rolling average (adjust window size as needed)
    # Define the significant inflation periods
    ...
    for column in merged_data_mom.columns:
        if 'Rolling_Correlation_' in column:
            # Extract the ticker name from the column name
            ticker_name = column.replace('Rolling_Correlation_', '').replace('_Return_Rate_MoM', '')
    ...
    ax1.set_title('Rolling Month-over-Month Correlation Between Inflation and Asset Returns')
    ax1.set_xlabel('Date')
    ax1.set_ylabel('Rolling Correlation (MoM)')
    ...
    for column in merged_data_yoy.columns:
        if 'Rolling_Correlation_' in column:
            # Extract the ticker name from the column name
            ticker_name = column.replace('Rolling_Correlation_', '').replace('_Return_Rate_YoY', '')
            # Plot the rolling correlation for each asset with simplified label
            ax2.plot(merged_data_yoy.index, merged_data_yoy[column], label=f'{ticker_name} ')

    # Customize the second subplot
    ax2.set_title('Rolling Year-over-Year Correlation Between Inflation and Asset Returns')
    ax2.set_xlabel('Date')
    ax2.set_ylabel('Rolling Correlation (YoY)')
